...
public class TestTanDouble {
    public static void main(String[] args) throws Exception {
        ...
        if (is_x86) {
            ArrayList<String> command_args = new ArrayList<String>(List.of(
                ...
                "-XX:CompileCommand=compileonly,"+TestEqualsWrapper.class.getName()+"::tan_double",
            ...
            // Verify llvm IR
            FileCheck checker = new FileCheck(dump_path, TestEqualsWrapper.class.getMethod("tan_double", double.class), false);
            // find compiled method
            checker.check("define hotspotcc double @\"compiler_jeandle_intrinsic_TestTanDouble$TestEqualsWrapper_tan_double");
            // check IR
        ...
        if (is_x86) {  // TODO: Add support for other architectures.
            ...
            command_args = new ArrayList<String>(List.of(
                ...
                "-Xlog:jeandle=debug", "-XX:+JeandleDumpIR",
                "-XX:JeandleDumpDirectory="+dump_path,
                "-XX:CompileCommand=compileonly,"+TestApproximateWrapper.class.getName()+"::tan_double",
                "-XX:-JeandleUseHotspotIntrinsics"));
            ...
            output.shouldHaveExitValue(0)
                .shouldContain("Method `static jdouble java.lang.Math.tan(jdouble)` is parsed as intrinsic");
            // Verify llvm IR
            checker = new FileCheck(dump_path, TestApproximateWrapper.class.getMethod("tan_double", double.class), false);
            // find compiled method
            checker.check("define hotspotcc double @\"compiler_jeandle_intrinsic_TestTanDouble$TestApproximateWrapper_tan_double");
            // check IR
            checker.checkNext("entry:");
            checker.checkNext("br label %bci_0");
            checker.checkNext("bci_0:");
            checker.checkNext("call double @llvm.tan.f64");
            checker.checkNext("ret double");
        }
    }
    ...
    static public class TestApproximateWrapper {
        ...
        public static void main(String[] args) {
            Random random = new Random();
            Asserts.assertLessThan(Math.abs(tan_double_verified(1.5d) - tan_double(1.5d)), epsilon);
            Asserts.assertLessThan(Math.abs(tan_double_verified(-1.5d) - tan_double(-1.5d)), epsilon);
            Asserts.assertEquals(tan_double_verified(Double.NaN), tan_double(Double.NaN));
            Asserts.assertEquals(tan_double_verified(Double.POSITIVE_INFINITY), tan_double(Double.POSITIVE_INFINITY));
            Asserts.assertEquals(tan_double_verified(Double.NEGATIVE_INFINITY), tan_double(Double.NEGATIVE_INFINITY));
            for (int i=0; i< 1000; i++) {
                double d = random.nextDouble();
                Asserts.assertLessThan(Math.abs(tan_double_verified(d) - tan_double(d)), epsilon);
            }
        }
